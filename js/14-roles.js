...
function _repPinLeer() {
  try {
    return JSON.parse(localStorage.getItem("rm_rep_pin") || "null");
  } catch {
    return null;
  }
}
function _repPinGuardar(pin, codigo) {
  try {
    localStorage.setItem("rm_rep_pin", JSON.stringify({
      pin: String(pin),
      codigo: codigo || ""
    }));
  } catch (e) {}
}
function _repPinBorrar() {
  try {
    localStorage.removeItem("rm_rep_pin");
  } catch (e) {}
}
  ...
  React.useEffect(() => {
    if (fase === "bio") {
      srBioVerificar().then(ok => {
        if (ok) onOk();else setFase("pin");
      }).catch(() => {
        setMsg("Usá tu PIN para entrar.");
        setFase("pin");
      });
    }
  }, []);
  const completar = v => {
    ...
    if (fase === "confirmar") {
      if (v === pinTmp) {
        ...
        setValor("");
        if (puedeBio && !bioOn && !(typeof bioRechazado === "function" && bioRechazado())) setFase("ofrecerBio");else onOk();
      } else {
...
function AppRepartidor({
  ...
  const todosRepartos = datos.repartos && datos.repartos.length > 0 ? datos.repartos : (() => {
    try {
      return JSON.parse(localStorage.getItem("rm_repartos_v1") || "[]");
    } catch {
      return [];
    }
  })();
  ...
  const sync = (overrides = {}) => {
    setDatos(prevDatos => {
      ...
      cloudLoad(uid, perfil.negocioId, perfil.codigo).then(function (fresh) {
        ...
        merged.clientes = mergeClientesPorUpd(prevDatos.clientes, overrides.clientes !== undefined ? overrides.clientes : prevDatos.clientes, fresh.clientes);
  ...
  const saveClientes = nc => sync({
    clientes: nc
  });
  const saveNoVisitas = nv => sync({
    noVisitas: nv
  });
  const savePlanilla = (key, val) => sync({
    planillas: {
      ...(datos.planillas || {}),
      [key]: val
    }
  ...
  const registrarVenta = (detalle, pago, montoPagado, saldoAplicado, envPrest, envDev, obs, opcionSaldo, mt2, sdOverride, transConfirmadaInicial) => {
    ...
    const nv = [...ventas, {
      ...
      envPrest: (envPrest || []).filter(e => e.prod && e.cant),
      envDev: (envDev || []).filter(e => e.prod && e.cant),
      mt2: mt2 || 0,
      ...calc,
      saldoDelta,
      montoTrans: esMixto ? tr : mt2 || 0,
      montoEfec: esMixto ? ef : 0,
      transConfirmada: !!transConfirmadaInicial,
      _upd: Date.now(),
      repartidor: perfil.nombre
    }];
    const clientesActualizados = todosClientes.map(x => x.id === c.id ? {
      ...x,
      saldo: (x.saldo || 0) + saldoDelta,
      _upd: Date.now()
    } : x);
    sync({
      ventas: nv,
  ...
  const registrarVentaLibre = (detalle, pago, montoPagado, saldoAplicado, envPrest, envDev, obs, op, mt2, sd, fechaKl) => {
    ...
    const nVenta = {
      ...
      dia: c.dia,
      fechaKey: fechaKl,
      fecha: new Date(fechaKl + "T12:00:00").toLocaleString("es-AR"),
      detalle,
      pago,
      obs: obs || "",
      saldoAplicado: saldoAplicado || 0,
    ...
    const clientesActualizados = todosClientes.map(x => x.id === c.id ? {
      ...x,
      saldo: (x.saldo || 0) + calc.saldoDelta,
      _upd: Date.now()
    } : x);
    sync({
      ventas: [...ventas, nVenta],
      clientes: clientesActualizados
    });
  };
  const eliminarVenta = ventaId => {
    ...
    const clientesActualizados = c2 ? clientes.map(x => x.id === c2.id ? {
      ...x,
      saldo: (x.saldo || 0) - v.saldoDelta,
      _upd: Date.now()
    } : x) : clientes;
    sync({
      ventas: nuevasVentas,
...
function PantallaActivacionRM({
  ...
  const [codigo, setCodigo] = React.useState(() => {
    try {
      ...
      if (hash.includes("activar?d=")) {
        ...
        if (data.c) {
          localStorage.setItem("_activacion_d", JSON.stringify(data));
          window.history.replaceState(null, "", window.location.pathname + "#activar");
          return data.c;
        }
      }
    } catch (_) {}
  ...
  const verificarCodigo = async () => {
    ...
    try {
      ...
      const lic = snap.data();
      if (lic.app && lic.app !== "reparto-multi") {
        setError("Este código no es para Reparto Multi.");
        setCargando(false);
        return;
      }
      if (lic.estado === "inactivo") {
        setError("Licencia desactivada. Contactá al soporte.");
        setCargando(false);
        return;
      }
      if (lic.estado === "pendiente") {
        setError("Licencia pendiente de activación. Contactá a Emma Soluciones.");
        setCargando(false);
        return;
      }
      if (lic.estado === "usado") {
  ...
  const completarActivacion = async () => {
    if (!nombre.trim() || !celular.trim() || !email.trim() || !pinIngresado.trim()) {
      setError("Completá todos los campos");
      return;
    }
    if (!/\S+@\S+\.\S+/.test(email)) {
    ...
    try {
      ...
      if (licData.email && licData.email.trim().toLowerCase() !== email.trim().toLowerCase()) {
        setError("El email no coincide con el registrado. Contactá al administrador.");
        setCargando(false);
        return;
      }
      if (licData.celular && licData.celular.trim() !== celular.trim()) {
        setError("El celular no coincide con el registrado. Contactá al administrador.");
        setCargando(false);
        return;
      }
      if (String(licData.pin) !== pinIngresado.trim()) {
        setError("El PIN no es correcto. Revisá el que te enviaron.");
        setCargando(false);
        return;
      }
      ...
      try {
        const negSnap = await window.db.collection("negocios").where("codigoActivacion", "==", cod).limit(1).get();
...
function RepartidoresPanel({
  ...
  React.useEffect(() => {
    if (!negocioId) return;
    listarRepartidores(negocioId).then(r => {
      setRepartidores(r);
      setCargando(false);
    });
  ...
  const resetearDispositivo = async (uid, nombre) => {
    ...
    if (window.db && reparto.codigo) {
      try {
        await window.db.collection("_codigos").doc(reparto.codigo).update({
          deviceId: null,
          activado: false
        });
        alert(`✅ Dispositivo de ${nombre} reseteado.\nCompartile el enlace 📤 para que active de nuevo.`);
        setRepartidores(r => r.map(x => x.uid === uid ? {
          ...x,
          deviceId: null
        } : x));
        return;
    ...
    return;
    if (false) {
      // bloque original desactivado — ya no se necesita
      ...
      try {
        ...
        if (!snap1.empty) {
          await Promise.all(snap1.docs.map(d => d.ref.update({
            deviceId: null,
            activado: false
          })));
          reseteado = true;
        ...
        try {
          await db.collection("negocios").doc(negocioId).collection("repartidores").doc(uid).update({
            deviceId: null,
            activado: false
          });
          reseteado = true;
        ...
        try {
          ...
          if (!snap3.empty) {
            await Promise.all(snap3.docs.map(d => d.ref.update({
              deviceId: null,
              activado: false,
              estado: "pendiente"
            })));
        ...
        try {
          ...
          if (repartidor?.codigo) {
            ...
            if (snap4.exists) await snap4.ref.update({
              deviceId: null,
              activado: false,
              estado: "pendiente"
            });
        ...
        try {
          ...
          if (!snap5.empty) {
            await Promise.all(snap5.docs.map(d => d.ref.update({
              deviceId: null,
              activado: false
            })));
            reseteado = true;
