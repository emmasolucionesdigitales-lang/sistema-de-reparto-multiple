...
function OnboardingRoles({uid, email, onListo}) {
  ...
  const crearCuentaDueno = async () => {
    if(!nombre.trim()||!negocio.trim()){setError("Completa todos los campos");return;}
    setCargando(true); setError("");
    try {
      const negocioId = await crearNegocio(uid, negocio.trim());
      const p = {rol:"dueno", negocioId, nombre:nombre.trim(), email, negocioNombre:negocio.trim()};
  ...
  const unirseConCodigo = async () => {
    if(!codigo.trim()){setError("Ingresa el codigo");return;}
    setCargando(true); setError("");
  ...
  if(modo==="") return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"var(--color-background-primary)",padding:24,gap:14}}>
      ...
      <div style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:8,textAlign:"center"}}>Como vas a usar la app?</div>
      <button style={{...s.btnPrimary,width:"100%",maxWidth:320,padding:16,fontSize:15,borderRadius:12}} onClick={()=>setModo("dueno")}>
         Soy dueno — crear mi negocio
      </button>
      <button style={{...s.btn,width:"100%",maxWidth:320,padding:16,fontSize:15,borderRadius:12}} onClick={()=>setModo("repartidor")}>
         Soy repartidor — tengo un codigo
      </button>
    </div>
  );

  if(modo==="dueno") return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"var(--color-background-primary)",padding:24}}>
      <div style={{width:"100%",maxWidth:380}}>
        ...
        <button style={{...s.btnPrimary,width:"100%",padding:14,fontSize:15}} onClick={crearCuentaDueno} disabled={cargando}>
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"var(--color-background-primary)",padding:24}}>
      <div style={{width:"100%",maxWidth:380}}>
        ...
        <div style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:20}}>El dueno te tiene que dar un codigo de 6 letras.</div>
        <label style={s.label}>Codigo de invitacion</label>
...
function AppRepartidor({uid, perfil, onSalir: onSalirProp}) {
  ...
  const _diasSemana = ["Domingo","Lunes","Martes","Miercoles","Jueves","Viernes","Sabado"];
  ...
  const [diaClienteActual, setDiaClienteActual] = React.useState(diaActual); // ahora si esta definido
  ...
  // Clientes de este repartidor del dia de HOY
  ...
  const irAlSiguiente = () => {
    ...
    // Buscar siguiente cliente del dia
    ...
    // Si no hay mas clientes, pasar a prospectos
...
function PantallaActivacionRM({onActivado}) {
  ...
  const verificarCodigo = async () => {
    ...
    if(!cod){ setError("Ingresa el codigo"); return; }
    setCargando(true); setError("");
    try {
      // 6 letras = codigo de repartidor (invitacion del dueno)
      ...
      // Codigo de dueno (ej: RM-XXXX)
      const snap = await window.dbLicencias.collection("licencias").doc(cod).get();
      if(!snap.exists){ setError("Codigo invalido. Verifica que este bien escrito."); setCargando(false); return; }
      const lic = snap.data();
      if(lic.app && lic.app !== "reparto-multi"){ setError("Este codigo no es para Reparto Multi."); setCargando(false); return; }
      if(lic.estado === "inactivo"){ setError("Licencia desactivada. Contacta al soporte."); setCargando(false); return; }
      if(lic.estado === "pendiente"){ setError("Licencia pendiente de activacion. Contacta a Emma Soluciones."); setCargando(false); return; }
      if(lic.estado === "usado" && lic.deviceId && lic.deviceId !== getDeviceId()){ setError("Este codigo ya fue usado en otro dispositivo."); setCargando(false); return; }
  ...
  const completarActivacion = async () => {
    if(!nombre.trim()||!celular.trim()||!email.trim()){ setError("Completa todos los campos"); return; }
    ...
    try {
      ...
      const profile = {
        rol:"dueno", negocioId:licData.negocioId||cod, pin:licData.pin,
      ...
      onActivado(profile);
    } catch(e){ setError("Error al activar. Intenta de nuevo."); }
    setCargando(false);
  };
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32,minHeight:"100vh",gap:16,background:"var(--color-background-primary)"}}>
      ...
      {paso===1&&<>
        <p style={{fontSize:14,color:"var(--color-text-secondary)",textAlign:"center",maxWidth:280,lineHeight:1.5,margin:0}}>
          Ingresa el codigo de activacion que recibiste.<br/>
          <span style={{fontSize:12,color:"var(--color-text-tertiary)"}}>Duenos: codigo RM-XXXX · Repartidores: codigo de 6 letras</span>
        </p>
        <div style={{width:"100%",maxWidth:320}}>
          <label style={s.label}>Codigo de activacion</label>
          <input style={{...stInp,textAlign:"center",fontSize:18,letterSpacing:3,textTransform:"uppercase"}}
            placeholder="RM-XXXX o codigo de 6 letras"
      ...
      {paso===2&&<>
        <p style={{fontSize:14,color:"var(--color-text-success)",textAlign:"center",maxWidth:280,lineHeight:1.5,margin:0}}>
          ✓ Codigo valido. Completa tus datos para activar.
        </p>
        <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div><label style={s.label}>Numero de celular *</label>
...
function RepartidoresPanel({negocioId, clientes}) {
  ...
  const resetearDispositivo = async (uid, nombre) => {
    if(!window.confirm(`Resetear dispositivo de ${nombre}?\n\nPodra activar la app de nuevo con su codigo en cualquier telefono.`)) return;
    ...
    // Si no hay funcion o fallo, intentar directamente con window.db
    if(window.db && reparto?.codigo) {
      try {
        await window.db.collection("_codigos").doc(reparto.codigo).update({deviceId:null, activado:false});
        alert(`✅ Dispositivo de ${nombre} reseteado.`);
    ...
    let reseteado = false;
    try {
      // Intento 1: coleccion global "repartidores" por uid
      ...
      // Intento 4: coleccion "invitaciones" por deviceId
      try {
      ...
      // Intento 5: coleccion "codigos" por uid (variante)
      try {
      ...
      if(reseteado){
        alert(`✅ Dispositivo de ${nombre} reseteado.\nYa puede activar la app de nuevo con su codigo en cualquier telefono.`);
      } else {
        alert(`⚠️ Reset ejecutado pero no se encontro el registro del dispositivo en la base de datos.\n${nombre} puede intentar activar de nuevo — si falla con "codigo ya usado", contacta soporte.`);
      }
      setRepartidores(r=>r.map(x=>x.uid===uid?{...x,deviceId:null}:x));
    } catch(e) {
    ...
    <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{...s.card,margin:0}}>
        ...
        {!cargando && repartidores.length===0 && (
          <div style={{fontSize:13,color:"var(--color-text-tertiary)"}}>Aun no hay repartidores activos.</div>
        )}
        {repartidores.map(r=>(
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              <button style={{fontSize:11,padding:"4px 10px",borderRadius:8,border:"0.5px solid var(--color-text-success)",background:"var(--color-background-success)",color:"var(--color-text-success)",cursor:"pointer",fontWeight:500}}
                onClick={()=>{
                  ...
                  if(navigator.share){navigator.share({title:"Activacion reparto",text:"Abri este enlace para activar la app",url:enlace}).catch(()=>{navigator.clipboard?.writeText(enlace);alert("Enlace copiado: "+enlace);});}
                  else{navigator.clipboard?.writeText(enlace).then(()=>alert("✅ Enlace copiado.\nPegalo en WhatsApp y enviaselo al repartidor.")).catch(()=>alert("Enlace: "+enlace));}
                }}>
                 Compartir
              </button>
      ...
      {/* Invitaciones pendientes (creadas desde Panel del dueno, aun no usadas) */}
      {invitesPendientes.length>0&&(
        <div style={{...s.card,margin:0,borderLeft:"3px solid #f5b942"}}>
          ...
          <div style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:10}}>
            El repartidor aun no uso su codigo para ingresar.
          </div>
          {invitesPendientes.map(inv=>(
            <div key={inv.codigo} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
              ...
              <button style={{...s.btn,fontSize:11,padding:"4px 10px"}} onClick={()=>{
                const txt=`Codigo para ingresar a Sistema de Reparto: ${inv.codigo}`;
                if(navigator.share){navigator.share({title:"Codigo de acceso",text:txt});}
                else{navigator.clipboard?.writeText(txt);alert("Codigo copiado: "+inv.codigo);}
              }}> Compartir</button>
            </div>
          ))}
        </div>
      ...
      {/* Mensaje: crear repartidores desde Panel del dueno */}
      <div style={{...s.card,margin:0,background:"var(--color-background-info)",border:"0.5px solid var(--color-border-info)"}}>
        <div style={{fontSize:13,color:"var(--color-text-info)",fontWeight:600,marginBottom:6}}> Como agregar un repartidor?</div>
        <div style={{fontSize:12,color:"var(--color-text-secondary)",lineHeight:1.6}}>
          Anda a <b>Panel del dueno → + Nuevo reparto</b>, completa el nombre y el codigo se genera solo.<br/>
          Comparti ese codigo con el repartidor para que entre a la app.
        </div>
      </div>
    </div>
  );
...
function NuevoClienteForm({sectores, diaActual, onGuardar, onVolver}) {
  var diasOpc = ["Lunes","Martes","Miercoles","Jueves","Viernes","Sabado","Domingo"];
  var init = {
  ...
  var guardar = function() {
    if(!datos.nombre || !datos.nombre.trim()){ alert("Ingresa el nombre del cliente"); return; }
    onGuardar(datos);
  };

  var campos = [
    ["barrio","Barrio"], ["calle","Calle"], ["nro","Numero"],
    ["manzana","Manzana"], ["lote","Lote"],
    ["telefono","Telefono (sin 0 ni 15)"], ["maps","Link Google Maps"]
  ];
  ...
  return React.createElement("div", {style: s.screen},
    ...
    React.createElement("div", {style: {padding:16, display:"flex", flexDirection:"column", gap:10}},
      React.createElement("div", null,
        ...
        React.createElement("input", {style: s.input, value: datos.nombre, placeholder: "Ej: Juan Garcia",
          onChange: function(e){ set("nombre", e.target.value); }})
      ),
      React.createElement("div", null,
        React.createElement("label", {style: s.label}, "Dia de reparto"),
      ...
      React.createElement("div", {style: {display:"flex", gap:10}},
        [["sifon","Sifon"],["bidon10","10L"],["bidon20","20L"]].map(function(par){
