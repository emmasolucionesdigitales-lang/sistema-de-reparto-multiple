...
function NuevaVenta({cliente,productos,fecha,onGuardar,onNoEsta,onNoQuiere,onVolver,onSaltar,ventasCliente,progressData}) {
  ...
  const [cantidades,setCantidades]=useState(()=>{
    ...
    if(ultimaConProd){
      ...
      det.forEach(d=>{if(!d._esDispRoto&&(d.nombre in m)&&nombresEntrega.includes(d.nombre))m[d.nombre]=d.cantidad;});
    }
    return m;
  });
      ...
      <div style={{padding:16}}>
        ...
        <button style={{...s.btnPrimary,marginBottom:10,opacity:detalle.length===0?0.45:1}} disabled={detalle.length===0} onClick={()=>{
          ...
          if(pago==="mixto"){
            ...
            else onGuardar(detalle,"transferencia",String(tr),saldoApl,envPrest,envDev,obs,"mixto_tr",ef,saldoDelta,transConfMixto);
          } else {
