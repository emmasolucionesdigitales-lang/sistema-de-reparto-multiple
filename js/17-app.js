...
function AppPrincipal({
  ...
  const eliminarVenta = ventaId => {
    ...
    saveClientes(prev => aplicarMovimientoEnvases(prev, ventas, v.clienteId, v.envDev, v.envPrest).map(x => x.id === v.clienteId ? {
      ...x,
  ...
  const deshacerUltimaVenta = () => {
    ...
    saveClientes(prev => {
      let out = prev;
      ventasBorradas.forEach(vb => {
        out = aplicarMovimientoEnvases(out, ventas, vb.clienteId, vb.envPrest, vb.envDev);
      });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    dia: diaActual,
    onIrClientes: () => {
      const yaIniciado = fechaActual && planillas[claveDiaReparto(diaActual, fechaActual, repartoActual?.id)]?.iniciado;
