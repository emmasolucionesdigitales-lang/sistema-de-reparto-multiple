...
function AppPrincipal({uid, email: emailProp, perfil}) {
  ...
  useEffect(() => {
    ...
    cloudLoad(uid, negocioId).then(function(data) {
      ...
      if (data.planillas)          {
        ...
        const planillasLocales = (()=>{ try{ return JSON.parse(localStorage.getItem("rm_planillas_v1")||"{}"); }catch{ return {}; } })();
      ...
      if (data.noVisitas?.length)  {
        ...
        const noVisitasLocales = (()=>{ try{ return JSON.parse(localStorage.getItem("rm_novisitas_v1")||"[]"); }catch{ return []; } })();
    ...
    const _refreshInterval = setInterval(()=>{
      cloudLoad(uid, negocioId).then(function(data){
        ...
        if(data.ventas?.length) setVentasRaw(prev => {
          ...
          const porId = {}; prev.forEach(v=>{ porId[v.id]=v; });
          let huboCambios = false;
          data.ventas.forEach(v=>{
            const local = porId[v.id];
            if(!local){ porId[v.id]=v; huboCambios=true; return; }
            const uL = Number(local._upd)||0, uN = Number(v._upd)||0;
            if(uN > uL){ porId[v.id]=v; huboCambios=true; }
          });
          return huboCambios ? Object.values(porId) : prev;
        });
        if(data.clientes?.length) setClientes(prev => {
          const porId = {}; prev.forEach(c=>{ porId[c.id]=c; });
          let huboCambios = false;
          data.clientes.forEach(c=>{
            const local = porId[c.id];
            if(!local){ porId[c.id]=c; huboCambios=true; return; }
            const uL = Number(local._upd)||0, uN = Number(c._upd)||0;
            if(uN > uL){ porId[c.id]=c; huboCambios=true; }
          });
          return huboCambios ? Object.values(porId) : prev;
        });
        if(data.planillas) setPlanillas(prev => {
          ...
          Object.keys(data.planillas).forEach(dia=>{
            const nub = data.planillas[dia]; const loc = prev[dia];
            if(!loc){ merged[dia]=nub; huboCambios=true; return; }
            const uL = Number(loc?._upd)||0, uN = Number(nub?._upd)||0;
            if(uN > uL){ merged[dia]=nub; huboCambios=true; }
          });
  ...
  React.useEffect(()=>{
    ...
    if(ventas && ventas.length){
      ...
      if(!viejas.length){ localStorage.setItem("rm_archivado_ventas_hasta", limiteKey); }
      if(viejas.length){
            ...
            if(recientes.length < ventas.length){
              ...
              if(deudores.length && window.enviarEmailBrevoRM && emailProp){
                ...
                window.enviarEmailBrevoRM({
                  to: emailProp, toName: perfil?.negocio||"",
    ...
    if(noVisitas && noVisitas.length){
      ...
      if(!viejasNV.length){ localStorage.setItem("rm_archivado_novisitas_hasta", limiteKey); }
      if(viejasNV.length){
  ...
  const syncData = (overrides={}) => {
    ...
    const data = { ...prevData, ...overrides, noVisitas: overrides.noVisitas!==undefined ? overrides.noVisitas : (prevData.noVisitas||[]), prospectos: overrides.prospectos!==undefined ? overrides.prospectos : (prevData.prospectos||[]), recordatorios: overrides.recordatorios!==undefined ? overrides.recordatorios : (prevData.recordatorios||[]), mantVeh: overrides.mantVeh||mantVehActual, histPrecios: overrides.histPrecios||histPreciosActual, zonasReparto: overrides.zonasReparto||prevData.zonasReparto||{}, repartos: overrides.repartos||prevData.repartos||[], horaAvisoCierre: overrides.horaAvisoCierre || localStorage.getItem('rm_hora_notif_cierre') || '18:00', horasAvisoTrans: overrides.horasAvisoTrans || (()=>{try{return JSON.parse(localStorage.getItem('rm_horas_notif_trans')||'["13:00","19:00"]');}catch{return ['13:00','19:00'];}})(), diasAvisoMant: overrides.diasAvisoMant || (localStorage.getItem('rm_dias_notif_mant')||'3,2,1,0').split(',').map(n=>parseInt(n.trim(),10)).filter(n=>!isNaN(n)) };
  ...
  return (
    ...
    <div style={{...s.app, zoom: SCALES[scaleIdx]}}>
      ...
      {pantalla==="inicioReparto"  && <InicioReparto dia={diaActual} fecha={fechaActual} planilla={planillas[claveDiaReparto(diaActual,fechaActual,repartoActual?.id)]||planillaDiaVacia()} productos={productos} cargasDia={cargasDiaDe(repartoActual?.id)} stock={stockNorm}
        onGuardar={(p,descontar)=>{
          ...
          if(descontar && repartoActual){
            ...
            saveCargasDiaDe(repartoActual.id, prev=>({...prev,[diaActual]:{soda,b10,b20}}));
          }
          irA("clientes");
      ...
      {pantalla==="clientes"       && <ListaClientes clientes={clientes.filter(c=>c.dia===diaActual&&(!repartoActual||c.repartoId===repartoActual.id))} dia={diaActual} fecha={fechaActual} ventas={ventas.filter(v=>v.fechaKey===fechaActual&&v.dia===diaActual&&(!repartoActual||clientes.find(c=>c.id===v.clienteId)?.repartoId===repartoActual.id))} todasVentas={ventas} noVisitas={(noVisitas||[]).filter(v=>v.dia===diaActual&&v.fecha===fechaActual&&(!repartoActual||clientes.find(c=>c.id===v.clienteId)?.repartoId===repartoActual.id))} onEditarCliente={(id,cambios)=>updateCliente(id,cambios)} onSeleccionar={c=>{setClienteId(c.id);irA("detalleCliente");}} onEntregar={c=>{setClienteId(c.id);irA("venta");}} onNuevoCliente={()=>irA("nuevoCliente")} onVolver={()=>irA("selectorFechaClientes")} onReordenar={lista=>{
        ...
        onVerProspecto={(p)=>{
          saveClientes(prev => prev.find(c=>c.id===p.id) ? prev : [...prev,{...p,saldo:p.saldo||0,_esProspecto:true}]);
          setClienteId(p.id);
          irA("detalleCliente");
        }}
  ...
  if(siguiente){ setClienteId(siguiente.id); irA("venta"); }
        ...
        onSaltar={()=>{
          const nv=[...(noVisitas||[]).filter(v=>!(v.clienteId===clienteId&&v.dia===diaActual&&v.fecha===fechaActual)),
            {clienteId,dia:diaActual,fecha:fechaActual,motivo:"salteado",_upd:Date.now()}];
          saveNoVisitas(nv);
          const clientesDia=clientes.filter(c=>c.dia===diaActual&&(!repartoActual||c.repartoId===repartoActual.id)).sort((a,b)=>(a.orden||9999)-(b.orden||9999));
      ...
      {pantalla==="nuevoCliente"   && <NuevoCliente diaActual={diaActual} repartoActual={repartoActual} onGuardar={(datos)=>{
          ...
          saveClientes(prevC => {
            ...
            return [...base,{...datos,id:Date.now(),saldo:0,dispenser:datos.dispenser||0,repartoId:repartoActual?.id||null}]
              .sort((a,b)=>DIAS.indexOf(a.dia)-DIAS.indexOf(b.dia)||(a.orden||9999)-(b.orden||9999));
          });
          irA("clientes");
        }} onVolver={()=>irA("clientes")} />}
      {pantalla==="historial" && <CargaHistorica clientes={clientes} productos={productos} onGuardar={(vts)=>{saveVentas(prev=>[...prev,...vts]);irA("menu");}} onVolver={()=>irA("menu")} />}
      ...
      {pantalla==="gestionClientes" && <GestionClientes clientes={clientes} repartos={repartos} repartoActual={repartoActual} onIrTab={irA} onReordenarTodo={(lista)=>saveClientes(lista)} onEditar={(id,cambios)=>{saveClientes(prev=>prev.map(c=>c.id===id?{...c,...cambios}:c));}} onEliminar={(id)=>{
      ...
      {pantalla==="clientesDormidos" && <React.Fragment><ClientesTabs activo="dormidos" onIr={irA}/><ClientesDormidos clientes={clientes} ventas={ventas} repartos={repartos} onVolver={()=>irA("gestionClientes")} onSeleccionar={c=>{setClienteId(c.id);irA("detalleDesdeGestion");}} onEditarCliente={(id,cambios)=>{saveClientes(prev=>prev.map(c=>c.id===id?{...c,...cambios}:c));}} onEliminar={eliminarCliente} onPerdida={registrarPerdida} /></React.Fragment>}
      ...
      {pantalla==="importarClientes" && <ImportarClientesExcel
        ...
        onGuardar={(nuevos)=>{saveClientes(prev=>[...prev,...nuevos]);irA("menu");}}
        onVolver={()=>irA("menu")}
      />}
      ...
      {pantalla==="agenda" && <AgendaScreen
        ...
        onNuevo={(datos)=>{
          ...
          saveRecordatorios(prev=>[...(prev||[]),{...datos,id:Date.now(),clienteId:c.id,clienteNombre:c.nombre,dia:c.dia,confirmado:false,paraRepartidor:datos.paraRepartidor||null}]);
        }}
        onIrCliente={(cId)=>{
        ...
        onVolver={()=>irA("menu")}
      />}
      {pantalla==="stock"          && <StockGeneral stock={stockNorm} setStock={(ns)=>{setStock(ns);syncData({stock:ns});}} clientes={clientes} setClientes={saveClientes} ventas={ventas} productos={productos} setProductos={saveProductos} cargasDia={cargasDiaDe(repartoActual?.id)} setCargasDia={(v)=>saveCargasDiaDe(repartoActual?.id,v)} planillas={planillas} repartos={repartos} perdidas={perdidas} registrarPerdida={registrarPerdida} onVolver={()=>irA("menu")} onResumen={()=>irA("resumen")} />}
      {pantalla==="fiadosPendientes" && <React.Fragment><ClientesTabs activo="fiados" onIr={irA}/><FiadosPendientes clientes={clientes} ventas={ventas} onEditarCliente={(id,cambios)=>updateCliente(id,cambios)} onCobrar={(cId,monto,pago)=>{
