...
function generarLinkInvitacionRepartidor(codigo) {
  const encoded = _srB64EncodeUtf8(JSON.stringify({
...
async function sincronizarInvitaciones(repartos, negocioId, licCodigo) {
  ...
  try {
    ...
    console.error("sincronizarInvitaciones:", e.message);
  }
}
...
async function canjearInvitacion(deviceId, email, codigo) {
  ...
  if (!window.auth || !window.auth.currentUser) return {
    ok: false,
    msg: "Todavía se está iniciando la conexión, esperá un segundo y probá de nuevo."
  };
  ...
  try {
    ...
    if (!snap.exists) return {
      ok: false,
      msg: "Código no encontrado. Pedile al dueño que abra su app — el código se activa automáticamente."
    };
    const d = snap.data();
    if (!d.activo) return {
      ok: false,
      msg: "Este reparto fue desactivado."
    };
    if (d.deviceId && d.deviceId !== "" && d.deviceId !== deviceId) return {
      ok: false,
      msg: "Este código ya está en uso en otro dispositivo. El dueño puede resetearlo."
    };
    ...
    await window.db.collection("repartidorUid").doc(miUid).set({
...
async function listarRepartidores(negocioId) {
  if (!window.db) return [];
  try {
    const snap = await window.db.collection("repartidores").where("negocioId", "==", negocioId).where("activado", "==", true).get();
    return snap.docs.map(d => ({
      ...d.data(),
      uid: d.data().deviceId || d.id,
      codigo: d.id
    }));
  } catch (e) {
    console.warn("listarRepartidores:", e);
    return [];
  }
}
async function listarInvitaciones(negocioId) {
  if (!window.db) return [];
  try {
    const snap = await window.db.collection("repartidores").where("negocioId", "==", negocioId).where("activo", "==", true).get();
    return snap.docs.filter(d => !d.data().activado).map(d => ({
      codigo: d.id,
...
async function eliminarRepartidor(uid) {
  ...
  try {
    ...
    console.error("eliminarRepartidor:", e);
  }
}
async function resetearDispositivoEnLicencia(uid, codigo) {
  ...
  try {
    ...
    await window.db.collection("repartidores").doc(codigo).update({
      deviceId: null,
      authUid: null,
      activado: false
    });
...
function VincularEmmaControl() {
  ...
  const guardarToken = t => {
    setToken(t);
    localStorage.setItem('sr_ec_token', t);
  };
  const resetToken = () => {
    setToken('');
    localStorage.removeItem('sr_ec_token');
    localStorage.removeItem('sr_ec_ultima');
  };
  const enviarResumenDia = async () => {
    if (!token || !db) {
      alert('Ingresá el código de Emma Control primero');
      return;
    }
    setEnviando(true);
    try {
      ...
      const movs = [...ventasHoy.map(v => ({
        id: 'venta_' + v.id,
        tipo: 'ingreso',
        monto: v.total || 0,
        descripcion: 'Venta reparto: ' + (v.cliente || ''),
        fecha: hoy,
        origen: 'reparto'
      })), ...gastosHoy.map(g => ({
        id: 'gasto_' + g.id,
        tipo: 'egreso',
        monto: g.monto || 0,
        descripcion: 'Gasto reparto: ' + (g.concepto || ''),
        fecha: hoy,
        origen: 'reparto'
      }))];
...
async function guardarLogoFirestore(codigo, logoB64) {
  if (!window.db || !codigo) return false;
  try {
    await window.dbLicencias.collection("licencias").doc(codigo).update({
      logo: logoB64
    });
    ...
    if (lic) {
      lic.logo = logoB64;
      localStorage.setItem("rm_licencia", JSON.stringify(lic));
    }
    return true;
  } catch (e) {
  ...
  const pedirYActivar = async () => {
    ...
    if (r === 'granted') {
      ...
      setResultado(ok ? {
        ...
        ok: false,
        msg: 'No se pudo completar la suscripción.'
      });
    }
    setProbando(false);
  };
...
function VistaClientesGeneral({
  ...
  const toggle = key => setAbiertos(o => ({
    ...o,
    [key]: !o[key]
  }));
  ...
  const clientesFiltrados = clientes.filter(c => {
    ...
    if (busq && !c.nombre.toLowerCase().includes(busqL) && !(c.barrio || "").toLowerCase().includes(busqL) && !(c.telefono || "").includes(busq)) return false;
    return true;
  });
  ...
  const grupos = React.useMemo(() => {
    ...
    clientesFiltrados.forEach(c => {
      ...
      if (!agrupado[repId]) agrupado[repId] = {
        rep,
        dias: {}
      };
...
function ImportarClientesExcel({
  ...
  const procesarExcel = async file => {
    ...
    if (!XLSX) {
      alert("Cargando libreria Excel, esperá unos segundos y volvé a intentar.");
      return;
    }
    ...
    for (let i = 0; i < Math.min(10, raw.length); i++) {
      ...
      if (r.some(c => c.includes("nombre") || c.includes("apellido"))) {
        headerRow = i;
        break;
      }
    }
    if (headerRow < 0) {
      alert("No encontre la fila de encabezados. Usa la plantilla oficial.");
      return;
    }
    ...
    for (let i = headerRow + 1; i < raw.length; i++) {
      ...
      if (!dia) {
        errs.push({
          ...
          problemas: [`Dia "${diaRaw}" no reconocido`]
        });
        continue;
      }
...
function SinGpsItem({
  ...
  const guardar = () => {
    if (!link.trim()) {
      alert("Pegá un link de Google Maps");
      return;
    }
    onGuardar(link.trim());
    setGuardado(true);
...
function usarInformes({
  ...
  const enviarSemanal = async fecha => {
    ...
    try {
      ...
      await window.enviarEmailBrevoRM({
        to: lic.email,
        toName: lic.negocio || lic.nombre || "",
        subject: `📊 Resumen semanal · ${fmtPesos(totalSem)}`,
        htmlContent
      });
      return true;
    } catch (e) {
      console.error("enviarSemanal:", e);
      return false;
    }
  };
